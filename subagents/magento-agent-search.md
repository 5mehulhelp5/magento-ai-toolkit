...
description: "Magento 2 catalog search specialist. Use when a user reports missing products in search or on category pages, zero-results, wrong relevance, red/yellow cluster, disk-watermark read-only, stuck catalogsearch_fulltext reindex, or asks about ES 7 → ES 8 / OpenSearch migration and managed-provider (AWS OpenSearch, Elastic Cloud) configuration."
...
|---------|--------|--------------------------|-----------|
| ES 7.17 | ES 8.x | Already on the Elastic stack, want to stay current, willing to enable security and move to Java 17+ | Licensing matters to you (ES 8 stays on Elastic License 2.0 / SSPL — not OSI-approved); your Magento version is <2.4.6 (no ES 8 support) |
| ES 7.17 | OpenSearch 2.x | Want Apache 2.0 licensing; already using AWS OpenSearch Service; distro is on a major cloud provider's managed offering | You have a heavy dependency on Elastic-exclusive features (X-Pack ML, advanced security) |
...
- **2.4.3–2.4.5**: ES 7 only
- **2.4.6**: ES 7, ES 8, OpenSearch 2.x — pick based on licensing preference
- **2.4.7**: ES 7 deprecated; ES 8 or OpenSearch 2.x recommended
- **2.4.8+**: ES 7 removed; ES 8 or OpenSearch 2.x required
