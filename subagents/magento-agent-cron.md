...
description: "Magento 2 cron specialist. Use when a user reports cron jobs not running, pending → missed pile-ups, error spikes, stuck running rows, cron_schedule bloat, consumer-runner failures, or distributed-cron contention; also for scaffolding new cron jobs (crontab.xml + handler, optional admin-editable schedule) and Adobe Commerce Cloud crons: questions."
...
- [`magento-cron.md`](../skills/magento-cron.md) — `crontab.xml`, `cron_groups.xml`, `cron_schedule` lifecycle, consumer runner, distributed cron, Adobe Commerce Cloud `crons:`
- [`magento-cli-command.md`](../skills/magento-cli-command.md) — area-aware CLI commands (often called from cron handlers)
- [`magento-infra.md`](../skills/magento-infra.md) — Supervisor/systemd consumer management as the alternative to `cron_consumers_runner`
...
## Step 4 — Adobe Commerce Cloud Crons

If `Step 1` identified Adobe Commerce Cloud, the wiring is different. There is no `bin/magento cron:install`.
